...
class DzyKeyBoardManger: NSObject {
    ...
    fileprivate func registerTextFieldViewClass(_ aClass: UIView.Type, didBeginEditingNotificationName : String, didEndEditingNotificationName : String) {
        NotificationCenter.default.addObserver(self, selector: #selector(self.textDidBeginEditing(_:)),    name: NSNotification.Name(rawValue: didBeginEditingNotificationName), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.textDidEndEditing(_:)),      name: NSNotification.Name(rawValue: didEndEditingNotificationName), object: nil)
    }
            ...
            if let unwrappedRootController = rootViewController {
                    ...
                    unwrappedRootController.modalPresentationStyle != UIModalPresentationStyle.pageSheet
                {
                    if let window = keyWindow() {
                        topViewBeginFrame?.origin = CGPoint(x: 0,y: window.frame.size.height-unwrappedRootController.view.frame.size.height)
                    } else {
...
//            textView.inputAccessoryView = getAccessoryView()
    ...
    @objc func doneClick(_ textField: UITextField) -> Void {
